...
# wandb.run.name = 'gpt-finetuning_001'  # wandb Run name set

run_name = f"gpt-finetuning_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
os.environ["WANDB_PROJECT"] = "Hanghae99"

# 기본 output_dir 설정
if not training_args.output_dir:
    training_args.output_dir = "/home/ubuntu/output"  # 원하는 기본 경로 설정
...
model = AutoModelForCausalLM.from_pretrained(
    ...
    force_download=True,  # 캐시된 모델 대신 새로 다운로드
    cache_dir=None  # 새로운 캐시 디렉토리 사용
)
...
eval_dataset = lm_datasets["validation"]  # Validation dataset 추가
...
trainer = Trainer(
    ...
    eval_dataset=eval_dataset,  # eval_dataset 설정
    tokenizer=tokenizer,
...
# Validation loss를 계산하고 log 남기기
eval_metrics = trainer.evaluate(eval_dataset=eval_dataset)
trainer.log_metrics("eval", eval_metrics)
trainer.save_metrics("eval", eval_metrics)

wandb.finish()
# wandb.log({"train/loss": metrics["train_loss"], "eval/loss": eval_metrics["eval_loss"]})